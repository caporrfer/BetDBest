...
public class WarehouseTest {
    ...
    static class ShipmentsManager {
        ...
        private ShipmentInfo findBestRoute(List<Warehouse> warehouses, Order order, BoxType box) throws NoSuitableWarehouseException {
            LocalDateTime deliveryDate = null, currentDeliveryDate = getDeliveryDateTime(order.getOrderDate(), departures.get(0).getShippingHours().get(0), times.get(0));
            ShipmentInfo shipInfo;
            float price = 0;
            int carriedTime = 0;
            float currentPrice = 100000;
            Warehouse currentWarehouse = null;

            for (Warehouse ware : warehouses) {              //Compruebo desde cual de los almacenes el envío es mas barato
                for (CarrierPricing state : pricings) {
                    for (CarrierTime time : times) {
                        if (ware.equals(state.getWarehouse())) {
                            if ((state.getTargetState().equals(order.getTargetState())) && (time.getTargetState().equals(order.getTargetState()))) {
                                price = box.getVolume() * state.getVolumePrice() + time.getCarrierTime() * EXPERIENCE_PRICE_BY_HOUR; //Tengo en cuenta la tasa extra por experiencia
                                if (currentPrice > price) {
                                    currentPrice = price;
                                    currentWarehouse = ware;
                                }
            ...
            for (DepartureTime dep : departures) {           //Compruebo cual de las fechas es óptima para que el producto llegue antes
                if ((dep.getTargetState().equals(order.getTargetState())) && (dep.getWarehouse().equals(currentWarehouse))) {
                    for (ShippingHour shipH : dep.shippingHours) {
                        for (CarrierTime time : times) {
                            if (time.getTargetState().equals(order.getTargetState())) {
                                deliveryDate = getDeliveryDateTime(order.getOrderDate(), shipH, time);
                                if (currentDeliveryDate.isAfter(deliveryDate)) { //Compruebo la mejor fecha hasta el momento con la que sigue, y si es mejor la sobreasigno
            ...
            shipInfo = new ShipmentInfo(order, currentWarehouse, currentDeliveryDate, box.getBoxType(), currentPrice);
            return shipInfo;
        }

        private BoxType findBestBoxType(Order order) throws NoSuitableBoxException {

            int minH = 0, minW = 0, minL = 0, w = 0;

            BoxType currentBox = boxTypes.get(0);

            for (Item item : items) {              //Compruebo que objeto tiene el pedido con el id
            ...
            for (BoxType btypes : boxTypes) {     //Miro la primera caja que le venga bien al pedido ya que como vienen ordenadas no hacen falta mas comprobaciones
                if ((btypes.getHeight() >= minH) && (btypes.getLength() >= minL) && (btypes.getWidth() >= minW) && (btypes.getMaxWeight() >= w)) {
                    currentBox = btypes;
                    break;
                }
            }
            //NO HACE FALTA YA QUE LAS BOXES SE CARGAN ORDENADAS DE MENOR A MAYOR
            /*  for (BoxType btype : boxTypes) {

                if (currentBox.getHeight() > btype.getHeight() && btype.getHeight() >= minH) {
                    if (currentBox.getLength() > btype.getLength() && btype.getLength() >= minL) {
                        if (currentBox.getLength() > btype.getLength() && btype.getLength() >= minL) {
